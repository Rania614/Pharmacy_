...
function DailyOffers() {
  ...
  const nextMobileSlide = () => {
    setMobileIndex((prev) => (prev + 1) % offers.length);
  };
  ...
  const handleAddToCart = (e, offer) => {
    e.preventDefault();
    e.stopPropagation();
    // Create a product-like object from offer
    const product = {
      id: offer.id,
            ...
            >
              {offers.map((offer) => (
                <div key={offer.id} className="min-w-full px-2">
                  <Link to={offer.path} className="block">
                    <div className="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden">
                      {/* Discount Badge */}
                      <div className="relative">
                        <div className="absolute top-4 left-4 z-10 bg-red-500 text-white px-4 py-1.5 rounded-full text-sm font-bold">
                          {offer.discount}%-
                        </div>
                        
                        {/* Product Icon - Large and Centered */}
                        <div className="p-8 flex items-center justify-center bg-gradient-to-br from-primary/5 to-primary/10 min-h-[200px]">
                          <div className="text-9xl transform transition-transform duration-300">
                            {offer.image}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="p-6 bg-white">
                        ...
                        <button
                          ...
                          className="w-full bg-primary text-white py-4 rounded-full font-bold text-lg flex items-center justify-center gap-2 hover:bg-primary/90 transition shadow-md"
                        >
            ...
            {offers.map((_, index) => (
              <button
                ...
                className={`h-2 rounded-full transition-all ${
                  mobileIndex === index ? 'bg-primary w-8' : 'bg-gray-300 w-2'
                }`}
              />
            ))}
          </div>
